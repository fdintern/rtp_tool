...
with col5:
    st.metric(label="IHT estate (end)", value=f"£{final.iht_estate:,.0f}")
with col6:
    st.metric(label="IHT on death (end)", value=f"£{final.iht:,.0f}", help="IHT payable if death occurred at end of projection. SIPPs are included in the estate from 2027 (Autumn Budget 2024).")
...
if True:
    ...
    s.background_gradient(cmap='Oranges', subset=['income_tax_rate_1', 'income_tax_rate_2', 'cgt_rate', 'iht_rate'], vmin=0, vmax=1)
...
if True:
    ...
    cdf = pd.DataFrame()
    cdf['Year'] = pd.to_datetime(df['year'], format='%Y')
    cdf['IHT Estate'] = df['iht_estate']
    cdf['IHT Payable'] = df['iht']
    cdf = cdf.melt('Year', var_name='Series', value_name='Value')

    chart3 = (
        alt.Chart(cdf, title='IHT on death (if death occurred in that year)')
        .mark_line()
        .encode(
            alt.X("Year:T", axis=alt.Axis(format="%Y", tickSize=0)),
            alt.Y("Value:Q", scale=yScale, axis=yAxis),
            alt.Color("Series:N", legend=legend),
        )
    )
...
* **IHT Est**: Estate subject to Inheritance Tax (ISA + GIA; SIPPs included from 2027 per Autumn Budget 2024)
* **IHT**: Inheritance Tax payable on death in that year (40% above the nil rate band: £325k single / £650k couple)
''')
